...
def parse_args():
    parser = argparse.ArgumentParser(description="Advanced Double-Run Shuffled RAG AI Agent")
...
def shuffle_choices(choices):
    """
    Shuffles choices deterministically based on choice texts.
    Returns: shuffled_choices_list, mapping_from_shuffled_to_original_letter
    """
    indexed_choices = list(enumerate(choices))
    # Simple deterministic shuffle to keep runs reproducible
...
def build_batch_prompt(batch_items, use_shuffled_choices=False):
    """
    Constructs a unified prompt containing a batch of questions.
    If use_shuffled_choices is True, it shuffles choices using the mapped values.
    """
    prompt = "Hãy trả lời các câu hỏi trắc nghiệm dưới đây. Đọc kỹ phần Ngữ cảnh đi kèm của từng câu hỏi.\n\n"
    
    # Few-shot Example to strictly enforce formatting
    prompt += (
        "--- VÍ DỤ MINH HỌA ---\n"
        "Đầu vào:\n"
...
def build_single_fallback_prompt(item, explain=True):
    """
    Constructs a detailed single-question prompt with optional Chain-of-Thought reasoning.
    """
    system_instructions = load_prompt_file("system_instructions.md")
    if not system_instructions:
        system_instructions = "Bạn là một siêu trợ lý AI chuyên nghiệp giải quyết câu hỏi trắc nghiệm."
        
    context_text = "\n".join(f"- {chunk}" for chunk in item["selected_chunks"])
    choices_str = ""
    for c_idx, choice in enumerate(item["choices"]):
        letter = chr(ord('A') + c_idx)
        choices_str += f"{letter}. {choice}\n"
    ...
    if explain:
        ...
        prompt += (
            "Chỉ đưa ra đáp án duy nhất dưới định dạng chính xác sau (không giải thích gì thêm):\n"
            "Đáp án cuối cùng: <chữ cái đáp án in hoa>\n"
            "Ví dụ: Đáp án cuối cùng: A"
        )
        
    return prompt

def append_to_csv(qid, answer, output_path, headers="qid,answer"):
...
def append_to_log_csv(qid, ans1, ans2, status, log_path):
    file_exists = os.path.exists(log_path) and os.path.getsize(log_path) > 0
    with open(log_path, "a", encoding="utf-8") as f:
        if not file_exists:
            f.write("qid,ans1,ans2,status\n")
        f.write(f"{qid},{ans1},{ans2},{status}\n")
...
def mock_choose_answer(qid, choices):
    """
    Deterministically choose a choice letter for a question based on its qid.
    """
...
def main():
    ...
    detailed_logs = []     # List of dicts for comparison logging
    conflicted_items = []  # Items deferred to Pass 2
    ...
    # Using the dynamically adjusted inference_batch_size
    ...
    for step in tqdm(range(num_inference_steps), desc="Processing PASS 1"):
        ...
        if not args.mock:
            ...
            for batch_items in step_batches:
                ...
                # Pre-fill assistant response to force formatting and prevent conversational/meta explanations
    ...
    # Write in original dataset order
